...
const CheckoutForm = ({ order }) => {
    ...
    const { price, userEmail, productName, _id } = order

    useEffect(() => {
        fetch('https://salty-island-81432.herokuapp.com/create-payment-intent', {
            method: 'POST',
            headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        if (intentError) {
            ...
            fetch(`https://salty-island-81432.herokuapp.com/order/${_id}`, {
                method: 'PATCH',
                headers: {
                    'content-type': 'application/json',
                    'authorization': `Bearer ${localStorage.getItem('accessToken')}`
